...
//  project_training
//
//  Created by Nguyen Dong Son on 4/11/18.
...
public struct TextSize {
    fileprivate struct CacheEntry: Hashable {
        ...
        fileprivate var hashValue: Int {
            return text.hashValue ^ Int(width) ^ Int(insets.top) ^
                Int(insets.left) ^ Int(insets.bottom) ^ Int(insets.right)
        }
    }
    ...
    public static func size(_ text: String, font: UIFont,
                            width: CGFloat, insets: UIEdgeInsets = UIEdgeInsets.zero) -> CGRect {
